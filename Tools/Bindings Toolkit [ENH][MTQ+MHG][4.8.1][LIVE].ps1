<#
.SYNOPSIS
    Bindings Toolkit for [Enhanced] MOZA MTQ + MHG. Menu-driven utility
...
.DESCRIPTION
    ...
    WORKFLOW (typical case -- right after loading the MTQ+MHG layout):
      1. Fully close Star Citizen and the RSI Launcher.
      2. Double-click "Bindings Toolkit [ENH][MTQ+MHG][4.8.1][LIVE].bat".
...
.EXAMPLE
    .\Bindings Toolkit [ENH][MTQ+MHG][4.8.1][LIVE].ps1
    Show the menu, prompt for channel as needed.

.EXAMPLE
    .\Bindings Toolkit [ENH][MTQ+MHG][4.8.1][LIVE].ps1 -Action MFD -Channel LIVE

.EXAMPLE
    .\Bindings Toolkit [ENH][MTQ+MHG][4.8.1][LIVE].ps1 -Action Invert -Channel PTU
#>
[CmdletBinding()]
param(
...
# =====================================================================
#  STICK-SPECIFIC DATA -- MOZA MTQ + MHG
#  When extending to another stick, change these two values and rename
#  the file. Everything below this block is shared logic.
# =====================================================================

$StickName = '[Enhanced] MOZA MTQ + MHG'

# MFD bind table. Each entry: name = SC action, input = vJoy button.
# Add multiTap = '2' for actions that need double-tap activation.
$Binds = @(
    @{ name = 'v_mfd_interact_cycle_backwards_short';      input = 'js2_button112' }
    @{ name = 'v_mfd_interact_cycle_forwards_short';       input = 'js2_button116' }
    @{ name = 'v_mfd_quick_action_repair_all';             input = 'js2_button5' }
    @{ name = 'v_mfd_select_view_comms_short';             input = 'js2_button53' }
    @{ name = 'v_mfd_select_view_configuration_short';     input = 'js2_button52' }
    @{ name = 'v_mfd_select_view_diagnostics_short';       input = 'js2_button55' }
    @{ name = 'v_mfd_select_view_ifcs_short';              input = 'js2_button54' }
    @{ name = 'v_mfd_select_view_resource_network_short';  input = 'js2_button56' }
    @{ name = 'v_mfd_select_view_scanning_short';          input = 'js2_button51' }
    @{ name = 'v_mfd_select_view_self_status_short';       input = 'js2_button49' }
    @{ name = 'v_mfd_select_view_target_status_short';     input = 'js2_button50' }
    @{ name = 'v_mfd_soft_select_cast_left_long';          input = 'js2_button109' }
    @{ name = 'v_mfd_soft_select_cast_right_long';         input = 'js2_button113' }
    @{ name = 'v_mfd_soft_select_mfd_1_long';              input = 'js2_button49' }
    @{ name = 'v_mfd_soft_select_mfd_2_long';              input = 'js2_button50' }
    @{ name = 'v_mfd_soft_select_mfd_3_long';              input = 'js2_button51' }
    @{ name = 'v_mfd_soft_select_mfd_4_long';              input = 'js2_button52' }
    @{ name = 'v_mfd_soft_select_mfd_5_long';              input = 'js2_button53' }
    @{ name = 'v_mfd_soft_select_mfd_6_long';              input = 'js2_button54' }
    @{ name = 'v_mfd_soft_select_mfd_7_long';              input = 'js2_button55' }
    @{ name = 'v_mfd_soft_select_mfd_8_long';              input = 'js2_button56' }
)
...
#  Refactored from the original Fix MFD Binds [ENH][MTQ+MHG].ps1.
# =====================================================================
...
function Invoke-FixMfd-Channel {
    ...
    if (-not (Test-Path -LiteralPath $Path)) {
        ...
        Write-Host "  Load the MTQ+MHG layout in-game first (Customization > Control Profiles)," -ForegroundColor Yellow
...
function Invoke-ClearAllBinds-Channel {
    ...
    Write-Host "    2. Customization > Control Profiles > Use this profile -- pick the MTQ+MHG layout." -ForegroundColor Cyan
...
function Test-LayoutXmlForChannel {
    # Returns @{ Present; Path; SizeKB; MTime } -- the MTQ+MHG layout file
    ...
    $found = @(Get-ChildItem -LiteralPath $mappings -Filter 'layout_ENH_MTQ_MHG_*_exported.xml' -ErrorAction SilentlyContinue)
    if ($found.Count -eq 0) {
        return @{ Present = $false; Reason = 'no layout_ENH_MTQ_MHG_*_exported.xml in mappings dir' }
    }
...
function Show-StackHealth {
    ...
    $shipped = Join-Path $ScriptRoot '..\Joystick Gremlin Profile [ENH][MTQ+MHG][4.8.1][LIVE][R14].xml'
    ...
    elseif ($vj.DeviceCount -lt 2) {
        Write-Host ("  vJoy: {0} loaded, {1} device(s) configured -- MOZA profile expects 2 (use vJoyConf to add a second)" -f $vVer, $vj.DeviceCount) -ForegroundColor Yellow
    }
    else {
...
function Invoke-ShowDiagnostic-Channel {
    ...
    if (-not (Test-Path -LiteralPath $Path)) {
        ...
        elseif ($slots.Count -eq 1) {
            Write-Host ("  vJoy slots in use: {0} only -- MOZA profile expects both js1 and js2" -f $slots[0]) -ForegroundColor Yellow
        }
        else {
