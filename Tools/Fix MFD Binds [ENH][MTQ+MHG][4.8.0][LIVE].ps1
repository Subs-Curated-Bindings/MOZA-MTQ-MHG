<#
.SYNOPSIS
    Re-applies the [Enhanced] MOZA MTQ + MHG MFD binds to Star Citizen's live
    actionmaps.xml.
...
.DESCRIPTION
    ...
    WORKFLOW:
      1. Load the [ENH][MTQ+MHG] layout in-game once (Customization > Control
...
.EXAMPLE
    .\Fix MFD Binds [ENH][MTQ+MHG][4.8.0][LIVE].ps1
    Auto-detect channels, prompt to pick.

.EXAMPLE
    .\Fix MFD Binds [ENH][MTQ+MHG][4.8.0][LIVE].ps1 -Channel PTU

.EXAMPLE
    .\Fix MFD Binds [ENH][MTQ+MHG][4.8.0][LIVE].ps1 -InstallRoot 'D:\Games\StarCitizen' -Channel PTU
#>
[CmdletBinding()]
param(
...
# ===================================================================
#  STICK-SPECIFIC BIND DATA -- MOZA MTQ MFD layout
#  Do not edit unless you are customizing your own MOZA MFD bindings.
# ===================================================================
$Binds = @(
    @{ name = 'v_mfd_interact_cycle_backwards_short';   input = 'js2_button64' }
    @{ name = 'v_mfd_interact_cycle_forwards_short';    input = 'js2_button68' }
    @{ name = 'v_mfd_quick_action_repair_all';          input = 'js2_button5'  }
    @{ name = 'v_mfd_select_view_comms_short';          input = 'js2_button35' }
    @{ name = 'v_mfd_select_view_configuration_short';  input = 'js2_button34' }
    @{ name = 'v_mfd_select_view_diagnostics_short';    input = 'js2_button37' }
    @{ name = 'v_mfd_select_view_ifcs_short';           input = 'js2_button36' }
    @{ name = 'v_mfd_select_view_resource_network_short'; input = 'js2_button38' }
    @{ name = 'v_mfd_select_view_scanning_short';       input = 'js2_button33' }
    @{ name = 'v_mfd_select_view_self_status_short';    input = 'js2_button31' }
    @{ name = 'v_mfd_select_view_target_status_short';  input = 'js2_button32' }
    @{ name = 'v_mfd_soft_select_cast_left_long';       input = 'js2_button61' }
    @{ name = 'v_mfd_soft_select_cast_right_long';      input = 'js2_button62' }
    @{ name = 'v_mfd_soft_select_mfd_1_long';           input = 'js2_button31' }
    @{ name = 'v_mfd_soft_select_mfd_2_long';           input = 'js2_button32' }
    @{ name = 'v_mfd_soft_select_mfd_3_long';           input = 'js2_button33' }
    @{ name = 'v_mfd_soft_select_mfd_4_long';           input = 'js2_button34' }
    @{ name = 'v_mfd_soft_select_mfd_5_long';           input = 'js2_button35' }
    @{ name = 'v_mfd_soft_select_mfd_6_long';           input = 'js2_button36' }
    @{ name = 'v_mfd_soft_select_mfd_7_long';           input = 'js2_button37' }
    @{ name = 'v_mfd_soft_select_mfd_8_long';           input = 'js2_button38' }
)
...
if (-not (Test-Path -LiteralPath $InstallRoot)) {
    ...
    if (-not (Test-Path -LiteralPath $entered)) {
        ...
        Write-Host "  .\Fix MFD Binds [ENH][MTQ+MHG][4.8.0][LIVE].ps1 -InstallRoot 'X:\path\to\StarCitizen'" -ForegroundColor Yellow
        exit 1
    }
    $InstallRoot = $entered
    Write-Host ""
